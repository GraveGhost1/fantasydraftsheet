...
function mergeLivePlayersWithSavedPlayers(livePlayers, savedPlayers, savedCustomRanks = null) {
  ...
  const merged = liveList.map((livePlayer) => {
    ...
    usedKeys.add(key);
    const savedPlayer = savedByKey.get(key);
    if (!savedPlayer) {
      return livePlayer;
    }

    const savedSnapshotRank = customRanks[key];
...
async function saveState({ awaitServer = false } = {}) {
  ...
  if (currentUsername) {
    ...
    const serverSave = saveStateToServer().catch((error) => {
      ...
      if (awaitServer) {
        showAppModal(
          'Failed to save to server. Your rankings are saved on this device for your account.',
...
async function loadStateFromServer() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.state) {
        loadedState = JSON.parse(data.state);
      }
    } else {
      const errorText = await response.text();
  ...
  if (!loadedState) {
    const localBackup = localStorage.getItem(getUserStorageKey(currentUsername));
    if (localBackup) {
      try {
        loadedState = JSON.parse(localBackup);
        console.log('[STATE] Loaded rankings from local backup for user:', currentUsername);
      } catch (error) {
        console.error('[STATE] Failed to parse local backup:', error);
      }
    }
  }
